...
const TrackControlMenu = (props: ITrackControlMenuProps) => {
  const menu = (
    <Menu onClick={({ key }) => props.addTrackControlProps(key)}>
      {props.ports.map((p) => (
        <Menu.Item key={p}>{`${props.port} & ${p}`}</Menu.Item>
      ))}
    </Menu>
  );
      ...
      <Dropdown overlay={menu} trigger={["click"]}>
        <Button size="small">
          ...
          <img
            ...
            src="/icons/icons8-bulldozer-96.png"
            alt="Track Control"
          />
...
const PortDetails = ({
    ...
    function portType() {
      if (hubDetails.hubHolder.ports.has(port)) {
        return hubDetails.hubHolder.hub
          ? portDeviceType(
              hubDetails.hubHolder.hub.getDeviceAtPort(port)?.type ||
                DeviceType.UNKNOWN
            )
          : "undefined";
      } else {
  ...
  const addMotorControlProps = () => {
    hubDetails.addMotorControlProps({
      motorPort: port,
      hubUuid: hubDetails.hubHolder.getUuid(),
    });
  };
    ...
    <div className="hub-details">
      ...
      <div style={{ flex: "2 0 70px", textAlign: "right" }}>
        <Tooltip title='Add a motor control for this port to the "Brick Remote" panel on the right'>
          <Button size="small" onClick={addMotorControlProps}>
            Add
            <img
              className="small-image"
              src="/icons/icons8-speedometer-100.png"
              alt="Motor Control"
            />
          </Button>
        </Tooltip>
    ...
    <div className="hub-details">
      ...
      <Tooltip title="Add a tilt indicator for this axis.">
        ...
        >
          ...
          <img
            className="small-image"
            src="/icons/icons8-hill-descent-control-96.png"
            alt="Tilt Control"
          />
        </Button>
      </Tooltip>
...
const HubDetails: React.FC<IHubDetailsProps> = (props: IHubDetailsProps) => {
  ...
  function renameHub(value: string) {
    if (value.length > 14) {
      message.error({
        content: "The hub name must not be longer then 14 characters",
        duration: 5,
      });
    } else {
    ...
    >
      ...
      >
        <Descriptions.Item label="UUID">
          {props.hubHolder.getUuid()}
        </Descriptions.Item>
        <Descriptions.Item label="Type">
          {props.hubHolder.getHubType()}
        </Descriptions.Item>
        {
          ...
          props.hubHolder.ports.has("TILT") ? (
            <Descriptions.Item label="Tilt X">
              <TiltDetails
                ...
                addTiltControlProps={() =>
                  props.addTiltControlProps({
                    axis: Axis.X,
                    hubUuid: props.hubHolder.getUuid(),
                  })
                }
              />
            </Descriptions.Item>
        ...
        <Descriptions.Item label="Port A">
          <PortDetails hubDetails={props} port="A" />
        </Descriptions.Item>
        <Descriptions.Item label="Port B">
          <PortDetails hubDetails={props} port="B" />
        </Descriptions.Item>
        <Descriptions.Item label="Port C">
          <PortDetails hubDetails={props} port="C" />
        </Descriptions.Item>
        <Descriptions.Item label="Port D">
          <PortDetails hubDetails={props} port="D" />
        </Descriptions.Item>
      ...
      <Button
        style={{ margin: "10px" }}
        onClick={() => disconnect(props.hubHolder)}
      >
        Disconnect
      </Button>
    </Card>
