...
const MotorControl: React.FC<IMotorControlProps> = (
  props: IMotorControlProps
) => {
  const step = 10;
  ...
  useEffect(() => {
    function driveMotor() {
      ...
      if (hub) {
        hub
          .setMotorSpeed(props.motorPort, inverted ? -motorSpeed : motorSpeed)
          .catch((err: unknown) =>
            console.log("Error while setting motorSpeed", err)
          );
      }
    }
